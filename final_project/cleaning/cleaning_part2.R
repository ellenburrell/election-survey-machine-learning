...
cleaned_data_2016 <- VOTER_Survey_December16_Release1 %>%
  ...
  select(starts_with(c("izip_2016","presvote16post_","ft","accurately_counted2","alcohol","smoke100","pid7","ideo","pew_religimp","starttime","endtime"))) %>%
  select(-c("presvote16post_t_2016","presvote16post_rnd_2016")) %>%
...
cleaned_data_2016_with_NA %>%
  group_by(presvote16post_2016) %>%
  summarise(n = n())

# Let's remove NA
cleaned_data_2016_with_NA <- cleaned_data_2016_with_NA %>%
  select(presvote16post_2016)


# Let's clean those who responded NA to outcome variable
# Let's factor presvote16post_2016
